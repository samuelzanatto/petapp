        ...
        const result = await prisma.$transaction(async (prisma) => {
            ...
            if (sightingId) {
                ...
                if (sighting) {
                    ...
                    await (0, notification_controller_1.sendPushNotification)(sighting.reportedBy, `${alert.pet.name} foi recuperado!`, `O dono de ${alert.pet.name} confirmou que o pet foi recuperado graças ao seu relato. Obrigado pela sua ajuda!`, {
                        type: 'PET_FOUND_THANKS',
                        alertId: id,
                        petId: alert.petId
                    });
...
exports.markPetAsFound = markPetAsFound;
// Função para obter estatísticas de recuperação de pets
const getPetRecoveryStats = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ message: 'Usuário não autenticado' });
...
exports.getPetRecoveryStats = getPetRecoveryStats;
