...
exports.searchPetsByRadius = exports.getFoundPets = exports.getLostPetAlertById = exports.getLostPets = exports.reportFoundPet = exports.reportLostPet = void 0;
...
const reportLostPet = async (req, res) => {
    try {
        const { petId, lastSeenLatitude, lastSeenLongitude, description, lastSeenLocation, reward } = req.body;
        ...
        const usersToNotify = nearbyUsers.filter(user => {
            ...
            const distance = calculateDistance(parseFloat(lastSeenLatitude), parseFloat(lastSeenLongitude), user.latitude, user.longitude);
...
exports.reportLostPet = reportLostPet;
const reportFoundPet = async (req, res) => {
    try {
        ...
        const potentialMatches = lostPetAlerts.filter(lostAlert => {
            ...
            const distance = calculateDistance(parseFloat(latitude), parseFloat(longitude), lostAlert.latitude, lostAlert.longitude);
            return distance <= DISTANCE_THRESHOLD;
        });
        res.status(201).json({
            alert,
        ...
        const usersToNotify = nearbyUsers.filter(user => {
            ...
            const distance = calculateDistance(parseFloat(latitude), parseFloat(longitude), user.latitude, user.longitude);
            return distance <= DISTANCE_THRESHOLD;
        });
...
exports.reportFoundPet = reportFoundPet;
const getLostPets = async (req, res) => {
    try {
        ...
        if (latitude && longitude) {
            ...
            const rad = parseFloat(radius) || 10; // Usar raio padrão maior (10km)
            ...
            if (!isNaN(lat) && !isNaN(lon)) {
                filteredAlerts = formattedAlerts.filter(alert => {
                    ...
                    // Calcular distância
                    const distance = calculateDistance(lat, lon, alert.latitude, alert.longitude);
                    // Log para debug
...
const getLostPetAlertById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const formattedAlert = {
            ...
            pet: {
                ...
                images: alert.pet.images
                    ? alert.pet.images.map(img => img.startsWith('http') ? img : `${API_URL}/${img.replace(/\\/g, '/')}`)
                    : [],
...
exports.getLostPetAlertById = getLostPetAlertById;
const getFoundPets = async (req, res) => {
    try {
        ...
        const formattedPets = foundPets.map(pet => {
            ...
            const petName = `${pet.species === 'DOG' ? 'Cachorro' : 'Gato'} ${pet.color ? pet.color : ''}`.trim();
            return {
                id: pet.id,
        ...
        if (latitude && longitude) {
            ...
            filteredPets = formattedPets.filter(pet => {
                if (!pet.latitude || !pet.longitude)
                    return false;
                const distance = calculateDistance(lat, lon, pet.latitude, pet.longitude);
                return distance <= rad;
            });
        }
                        ...
                        if (samePets.length > 0 && foundPet.imageUrl) {
                            ...
                            const petPromises = samePets.map(async (userPet) => {
                                ...
                                try {
                                    ...
                                    const comparisonResult = await (0, comparison_1.comparePetImages)(userPetLocalPath, foundPetLocalPath);
                                    return {
                                        petId: userPet.id,
                            ...
                            Promise.all(petPromises)
                                ...
                                const bestMatch = results.reduce((best, current) => current.similarity > best.similarity ? current : best, { similarity: 0 });
...
const searchPetsByRadius = async (req, res) => {
    try {
...
exports.searchPetsByRadius = searchPetsByRadius;
