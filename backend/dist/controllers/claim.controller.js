...
exports.updateClaimStatus = exports.cancelPetClaim = exports.completePetClaim = exports.verifyPetClaim = exports.listReceivedPetClaims = exports.listMyPetClaims = exports.getPetClaim = exports.createPetClaim = void 0;
const client_1 = require("@prisma/client");
const config_1 = require("../config");
const chat_controller_1 = require("./chat.controller");
const notification_controller_1 = require("./notification.controller");
const alertHelpers_1 = require("../utils/alertHelpers");
        ...
        claimant: claim.claimant ? {
            ...
            profileImage: claim.claimant.profileImage ?
                (claim.claimant.profileImage.startsWith('http') ?
                    ...
                    `${config_1.config.baseUrl}/${claim.claimant.profileImage}`) :
                null
        } : null,
        ...
        formattedClaim.alertDetails = {
            ...
            image: claim.foundAlert.image ?
                (claim.foundAlert.image.startsWith('http') ?
                    ...
                    `${config_1.config.baseUrl}/${claim.foundAlert.image}`) :
                null,
            ...
            user: claim.foundAlert.user ? {
                ...
                profileImage: claim.foundAlert.user.profileImage ?
                    (claim.foundAlert.user.profileImage.startsWith('http') ?
                        ...
                        `${config_1.config.baseUrl}/${claim.foundAlert.user.profileImage}`) :
                    null
            } : null
        };
    }
            ...
            user: claim.lostAlert.user ? {
                ...
                profileImage: claim.lostAlert.user.profileImage ?
                    (claim.lostAlert.user.profileImage.startsWith('http') ?
                        ...
                        `${config_1.config.baseUrl}/${claim.lostAlert.user.profileImage}`) :
                    null
            } : null,
            ...
            pet: claim.lostAlert.pet ? {
                ...
                primaryImage: claim.lostAlert.pet.primaryImage ?
                    (claim.lostAlert.pet.primaryImage.startsWith('http') ?
                        ...
                        `${config_1.config.baseUrl}/${claim.lostAlert.pet.primaryImage}`) :
                    null
            } : null
        };
    }
 ...
 */
const createPetClaim = async (req, res) => {
    try {
        ...
        if (req.files && Array.isArray(req.files)) {
            verificationImages = await Promise.all(req.files.map(async (file) => {
        ...
        const alertOwner = (0, alertHelpers_1.getAlertOwnerFromClaim)(claim);
        const alertOwnerId = (0, alertHelpers_1.getAlertOwnerIdFromClaim)(claim);
        if (alertOwner && alertOwnerId) {
            ...
            if (notificationImageUrl && !notificationImageUrl.startsWith('http')) {
                notificationImageUrl = `${config_1.config.baseUrl}/${notificationImageUrl}`;
            }
            // Enviar a notificação push
            try {
                await (0, notification_controller_1.sendFullNotification)({
                    userId: alertOwnerId,
                    senderId: userId,
                    type: 'CLAIM',
...
exports.createPetClaim = createPetClaim;
/**
 * Buscar detalhes de uma reivindicação específica
 */
const getPetClaim = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        if (!userId) {
        ...
        if (formattedResponse.verificationImages && formattedResponse.verificationImages.length > 0) {
            formattedResponse.verificationImages = formattedResponse.verificationImages.map((img) => img.startsWith('http') ? img : `${config_1.config.baseUrl}/${img}`);
        }
...
exports.getPetClaim = getPetClaim;
/**
 * Listar reivindicações feitas pelo usuário logado
 */
const listMyPetClaims = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ message: 'Usuário não autenticado' });
...
exports.listMyPetClaims = listMyPetClaims;
/**
 * Listar reivindicações para alertas criados pelo usuário
 */
const listReceivedPetClaims = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ message: 'Usuário não autenticado' });
...
exports.listReceivedPetClaims = listReceivedPetClaims;
/**
 * Verificar uma reivindicação de pet (aprovar ou rejeitar)
 */
const verifyPetClaim = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            if (action === 'APPROVE') {
                ...
                try {
                    await (0, chat_controller_1.createChatForApprovedClaim)(claim.id);
                }
                catch (chatError) {
...
exports.verifyPetClaim = verifyPetClaim;
/**
 * Marcar uma reivindicação como concluída (pet entregue)
 */
const completePetClaim = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        if (!userId) {
...
exports.completePetClaim = completePetClaim;
/**
 * Cancelar uma reivindicação
 */
const cancelPetClaim = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        if (!userId) {
        ...
        try {
            ...
            if (alertOwnerId) {
                await (0, notification_controller_1.sendFullNotification)({
                    userId: alertOwnerId,
                    senderId: userId,
                    type: client_1.NotificationType.CLAIM,
...
exports.cancelPetClaim = cancelPetClaim;
/**
...
const updateClaimStatus = async (req, res) => {
    try {
        ...
        const notificationUserId = isClaimant ? (0, alertHelpers_1.getAlertOwnerIdFromClaim)(claim) : claim.claimantId;
