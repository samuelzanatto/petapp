...
exports.deletePet = exports.updatePet = exports.getPetById = exports.getAllPets = exports.getUserPets = exports.createPet = void 0;
...
const createPet = async (req, res) => {
    try {
        const { name, nickname, species, breed, birthdate, age, furType, primaryColor, secondaryColor, distinguishingMarks, weight, size, hasDisability, disabilityDetails, takesRegularMedication, medicationDetails, hasSpecialDiet, dietDetails, isNeutered, microchipNumber, microchipLocation, veterinarianContact, temperament, isTrainedToCommands, reactsTo, description } = req.body;
        ...
        if (!Object.values(client_1.PetSpecies).includes(species)) {
...
const getPetById = async (req, res) => {
    try {
        ...
        const formattedPet = {
            ...
            images: pet.images
                ? pet.images.map(img => img.startsWith('http') ? img : `${API_URL}/${img.replace(/\\/g, '/')}`)
                : [],
...
const updatePet = async (req, res) => {
    try {
        ...
        if (hasDisability !== undefined)
            booleanFields.hasDisability = hasDisability === 'true';
        if (takesRegularMedication !== undefined)
            booleanFields.takesRegularMedication = takesRegularMedication === 'true';
        if (hasSpecialDiet !== undefined)
            booleanFields.hasSpecialDiet = hasSpecialDiet === 'true';
        if (isNeutered !== undefined)
            booleanFields.isNeutered = isNeutered === 'true';
        if (isTrainedToCommands !== undefined)
            booleanFields.isTrainedToCommands = isTrainedToCommands === 'true';
        // Converter valores numéricos
        const numericFields = {};
        if (age !== undefined)
            numericFields.age = age ? parseInt(age) : null;
        if (weight !== undefined)
            numericFields.weight = weight ? parseFloat(weight) : null;
        ...
        if (furType !== undefined)
            updateData.furType = furType;
        if (primaryColor !== undefined)
            updateData.primaryColor = primaryColor;
        if (secondaryColor !== undefined)
            updateData.secondaryColor = secondaryColor;
        if (distinguishingMarks !== undefined)
            updateData.distinguishingMarks = distinguishingMarks;
        if (size !== undefined)
            updateData.size = size;
        if (disabilityDetails !== undefined)
            updateData.disabilityDetails = disabilityDetails;
        if (medicationDetails !== undefined)
            updateData.medicationDetails = medicationDetails;
        if (dietDetails !== undefined)
            updateData.dietDetails = dietDetails;
        if (microchipNumber !== undefined)
            updateData.microchipNumber = microchipNumber;
        if (microchipLocation !== undefined)
            updateData.microchipLocation = microchipLocation;
        if (veterinarianContact !== undefined)
            updateData.veterinarianContact = veterinarianContact;
        if (temperament !== undefined)
            updateData.temperament = temperament;
        if (reactsTo !== undefined)
            updateData.reactsTo = reactsTo;
