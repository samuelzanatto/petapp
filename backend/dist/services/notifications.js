...
const initializeSocketIO = (httpServer) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('chat-message', async ({ chatRoomId, content }) => {
            try {
                ...
                if (chatRoom) {
                    ...
                    const isUserOnline = connectedUsers.has(otherUserId) && getUserSockets(otherUserId).some(s => s.rooms.has(`chat:${chatRoomId}`));
        ...
        socket.on('lost-pet-alert', async (data) => {
            try {
                ...
                if (nearbyUsers.length > 0) {
                    ...
                    await (0, notification_controller_1.sendBulkFullNotifications)({
                        userIds: nearbyUsers.map(user => user.id),
                        type: client_1.NotificationType.LOST_PET,
        ...
        socket.on('found-pet-alert', async (data) => {
            try {
                const userId = getUserIdFromSocket(socket);
                if (!userId) {
                    return socket.emit('error', { message: 'Não autorizado' });
                ...
                const nearbyLostPets = lostPets.filter(lostPet => {
                    if (!lostPet.latitude || !lostPet.longitude)
                        return false;
                    const distance = calculateDistance(latitude, longitude, lostPet.latitude, lostPet.longitude);
                ...
                if (uniqueUserIds.length > 0) {
                    ...
                    await (0, notification_controller_1.sendBulkFullNotifications)({
                        ...
                        type: client_1.NotificationType.FOUND_PET,
...
async function findNearbyUsers(latitude, longitude, radiusKm, excludeUserId) {
