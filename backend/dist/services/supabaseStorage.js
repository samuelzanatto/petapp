...
const bufferModeration_1 = require("./bufferModeration");
 ...
 */
async function uploadImage(buffer, bucket, mimeType, moderateContent = true) {
    // Moderar conteúdo se solicitado
    if (moderateContent) {
        const moderationResult = await (0, bufferModeration_1.moderateImageBuffer)(buffer, mimeType);
    ...
    for (const size of sizes) {
        ...
        const uploadPromise = supabase_1.supabase.storage
            .from(bucket)
 ...
 */
async function deleteImage(bucket, imageId) {
 ...
 */
async function downloadImage(bucket, path) {
